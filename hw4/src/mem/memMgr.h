...
public:
   MemMgr(size_t b = 65536) : _blockSize(b) {
      ...
      _activeBlock = new MemBlock<T>(0, _blockSize);
       // recycleList 有0~256種大小的recycle array
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      MemBlock<T> *a = _activeBlock;
      while(a ->getNextBlock() != NULL){
         MemBlock<T> *d = a;
         a = a->getNextBlock();
         delete d;
      }
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(p != NULL){
         ...
         p->setNextList(new  MemRecycleList<T> (n));
         return p->getNextList();
   }
   ...
   T* getMem(size_t t) {
      ...
      // TODO
      MemRecycleList <T > * getlist = getMemRecycleList(n);
      if(getlist -> _first != NULL){
         ret = getlist ->popFront();
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
         #endif // MEM_DEBUG
      }
