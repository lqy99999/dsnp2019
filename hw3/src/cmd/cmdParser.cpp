...
{
   // TODO...   
   map<const string, CmdExec*>::iterator iter;//加上map iterator
...
CmdParser::parseCmd(string& option)
{
   ...
   CmdExec* e = 0;
   string cmd;
   myStrGetTok(str, cmd, 0, ' ');
   e = getCmd(cmd);

   myStrGetTok(str, option, cmd.size(), '\n');
   // cout << option << endl;
...
CmdParser::getCmd(string cmd)
{
   CmdExec* e = 0;
   // TODO...
   map<const string, CmdExec*>::iterator it;//加上map iterator
   for(it = _cmdMap.begin(); it != _cmdMap.end(); it ++){
      string mcmd = it -> first;
      string option = it->second->getOptCmd();
      if((mcmd + option).size() < mcmd.size())
         continue;

      if(myStrNCmp(mcmd + option, cmd, mcmd.size()) == 0){
         //TEST
         // cout << it->first << endl;
         e = it->second;
         return e;
