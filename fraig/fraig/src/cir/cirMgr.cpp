...
CirMgr::printNetlist() const
{
   ...
   for(int i = 0; i < dfslist.size(); ++i){
      ...
      if(dfslist[i]->getTypeStr() == "PO"){
         unsigned input;
         if(dfslist[i]->fanin[0] != NULL){
            input = dfslist[i]->fanin[0]->getid();
         if(glist[input]->getTypeStr() == "UNDEF") cout << "*";
         if(dfslist[i]->getn1()) cout << "!";

         cout << input;
         }     
         // dynamic_cast<CirPOGate*>(dfslist[i])->getin();
      ...
      if(dfslist[i]->getTypeStr() == "AIG"){
         ...
         if(dfslist[i]->fanin[1] != NULL){
            input = dfslist[i]->fanin[1]->getid();
...
CirMgr::printFloatGates() const
{
   ...
   for(int i = 1; i < glist.size(); ++i){
      if(glist[i] != NULL && glist[i]->getTypeStr() == "AIG"){
         if(glist[i]->fanin[0]->getTypeStr() == "UNDEF" || glist[i]->fanin[1]->getTypeStr()=="UNDEF")
            hasffanin.push_back(glist[i]->getid());
      }
      if(glist[i] != NULL && glist[i]->getTypeStr() == "PO"){
         if(glist[i]->fanin[0]->getTypeStr() == "UNDEF")
            hasffanin.push_back(glist[i]->getid());
      }
   }
   ...
   for(unsigned int i = 1; i < glist.size(); ++i){
      // cout << "here2" << endl;
      if(glist[i] != NULL && glist[i]->fanout.empty() && glist[i]->getTypeStr() != "PO" && glist[i]->getTypeStr() != "UNDEF") 
   ...
   if(!hasffanin.empty()){
      ...
      for(int i = 0; i <  hasffanin.size(); ++i)
         cout << " " << hasffanin[i];
      cout << endl;
   }
...
CirMgr::printFECPairs() const
{
   bool hi;
   bool hey;
   for(unsigned a= 0; a < fecgroup.size(); a++){
      cout << "[" << a <<"] ";
      hi=((fecgroup[a])->operator[](0)%2 == 0)? false:true;
     for(unsigned i = 0; i < fecgroup[a]->size(); i++){
        hey = ((fecgroup[a])->operator[](i) % 2 == 0)? false:true;
        cout << ((hi ^ hey)? "!" : "");
       cout << ((hey)? (((fecgroup[a])->operator[](i) -1)>>1):(((fecgroup[a])->operator[](i)) >> 1));
       cout << " ";
      }
      cout << endl;
   }
...
CirMgr::writeAag(ostream& outfile) const
{
   ...
   cout << "c" << endl;
   cout << "AAG output by Ching-Yen Lee";
}
...
CirMgr::connect()
{
   ...
   for(int i  = 0; i < out.size(); ++i){ 
      ...
      if(f != NULL){
         ...
         CirGate* u = new CirUNDEFGate(idx, 0);
         out[i]->addfanin(u);
         glist[idx] = u;
         flist.push_back(idx);
         // cout << "here3" << endl;
   ...
   for(int i  = 0; i < aig.size(); ++i){  
      ...
      if(f != NULL){
         ...
         // cout << "here3" << endl;
         CirGate* u = new CirUNDEFGate(idx1,0);
         aig[i]->addfanin(u);
         glist[idx1] = u;
      ...
      if(g != NULL){
         ...
         // cout << "here5" << endl;
         CirGate* s = new CirUNDEFGate(idx2, 0);
         aig[i]->addfanin(s);
         glist[idx2] = s;
